...
<html lang="en">
  ...
  <body class="min-h-screen bg-gray-100 flex flex-col items-center justify-start p-8">
    ...
    <!-- Formulaire -->
    <form method="POST" name="form1" action="Paiement.jsp" class="bg-white p-8 rounded-lg shadow-md w-full max-w-md space-y-6">
      <div class="flex flex-col">
        <label class="text-gray-700 mb-2">ID :</label>
        <input readonly type="text" name="ID" value="<jsp:getProperty name='PaiementBean' property='ID'/>" class="border rounded px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400" />
      </div>
      <div class="flex flex-col">
        <label class="text-gray-700 mb-2">Montant paiement :</label>
        <input type="number" name="montant_paiement" value="<jsp:getProperty name='PaiementBean' property='montant_paiement'/>" class="border rounded px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400" />
      </div>
      <div class="flex flex-col">
        <label class="text-gray-700 mb-2">Date paiement :</label>
        <input type="date" name="date_paiement" value="<jsp:getProperty name='PaiementBean' property='date_paiement'/>" class="border rounded px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400" />
      </div>
      <div class="flex flex-col">
        <label class="text-gray-700 mb-2">Methode paiement :</label>
        <select name="methode_paiement" class="border rounded px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400">
          <option value="">-- Selectionnez une methode --</option>
          <option value="carte_bancaire" <%= "carte_bancaire".equals(PaiementBean.getMethode_paiement()) ? "selected" : "" %>>Carte bancaire</option>
          <option value="virement" <%= "virement".equals(PaiementBean.getMethode_paiement()) ? "selected" : "" %>>Virement</option>
          <option value="paypal" <%= "paypal".equals(PaiementBean.getMethode_paiement()) ? "selected" : "" %>>PayPal</option>
        </select>
      </div>
      <div class="flex flex-col">
        <label class="text-gray-700 mb-2">Statut paiement :</label>
        <select name="status_paiement" class="border rounded px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400">
          <option value="">-- Selectionnez un statut --</option>
          <option value="en_attente" <%= "en_attente".equals(PaiementBean.getStatus_paiement()) ? "selected" : "" %>>En attente</option>
          <option value="valide" <%= "valide".equals(PaiementBean.getStatus_paiement()) ? "selected" : "" %>>Valide</option>
          <option value="echec" <%= "echec".equals(PaiementBean.getStatus_paiement()) ? "selected" : "" %>>Echec</option>
        </select>
      </div>
      <div class="flex flex-col">
        <label class="text-gray-700 mb-2">Contribution :</label>
        <select name="contribution_contributionID" class="border rounded px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400">
